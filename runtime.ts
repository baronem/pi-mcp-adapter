import type { AgentToolUpdateCallback, ExtensionAPI, ExtensionCommandContext, ExtensionContext, ToolInfo } from "@earendil-works/pi-coding-agent";
...
export function installMcpRuntime(
...
): ActivatedMcpRuntime & { startSession(ctx: ExtensionContext): Promise<void> } {
  ...
  pi.on("session_shutdown", async () => {
    ...
    try {
      ...
      // The bootstrap and runtime intentionally share this key. Always clear it
      // when the activated runtime is torn down so reload cannot leave stale UI.
      currentState?.ui?.setStatus("mcp", undefined);
    }
  });
  ...
  const getMcpArgumentCompletions = (prefix: string): Array<{ value: string; label: string }> | null => {
    const normalized = prefix.trimStart();
    const argumentMatch = normalized.match(/^(\S+)\s+(.*)$/);
    if (!argumentMatch) {
      const subcommands = [
        ["reconnect", "Reconnect servers"],
        ["tools", "List all tools"],
        ["prompts", "List all MCP prompts"],
        ["setup", "Configure MCP servers"],
        ["logout", "Clear server credentials"],
        ["token", "Manage stored bearer tokens"],
  ...
  const handleMcpCommand = async (args: string, ctx: ExtensionCommandContext): Promise<void> => {
  ...
  // Register namespace-proxy tools when the deferred runtime activates so
  // tool-group validation sees the complete cached surface immediately.
  syncNamespaceTools(earlyConfig, earlyCache, initialDirectResult.reservedDirectNames, initialDirectResult.activeDirectNames);

  return { handleMcpCommand, getMcpArgumentCompletions, startSession };
}

export async function activateMcpRuntime(
  pi: ExtensionAPI,
  activationCtx: ExtensionCommandContext,
  options: McpAdapterOptions = {},
): Promise<ActivatedMcpRuntime> {
  const runtime = installMcpRuntime(pi, options);
  // Activation happens after Pi's session_start event, so explicitly initialize
  // the already-running session before delegating the command that activated us.
  await runtime.startSession(activationCtx);
  return runtime;
}

/** Runtime-only factory used by lifecycle tests and embedders that explicitly
 * manage session_start. The normal Pi entry point is the deferred bootstrap. */
export function createInstalledMcpRuntime(options: McpAdapterOptions = {}) {
  const savedOptions = structuredClone(options);
  return (pi: ExtensionAPI) => {
    const runtime = installMcpRuntime(pi, structuredClone(savedOptions));
    // This factory is intentionally runtime-only. It keeps direct runtime tests
    // and explicit embedders convenient without affecting the deferred entry.
    pi.registerCommand("mcp", {
      description: "Show MCP server status",
      getArgumentCompletions: runtime.getMcpArgumentCompletions,
      handler: runtime.handleMcpCommand,
    });
    return runtime;
  };
}

export default createInstalledMcpRuntime();
